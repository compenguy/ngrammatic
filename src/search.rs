        ...
        Warp<W>: Similarity + Copy,
    {
        ...
        let mut matches = ngram_counts_ref
            ...
            .flat_map(|(gram_number, ngram_id)| {
                self.key_ids_from_ngram_id(ngram_id).filter_map(move |key_id| {
                    if self.contains_any_ngrams(ngram_counts_ref.keys().take(gram_number), key_id) {
                    ...
                    let similarity = warp.similarity(
                        ngram_counts_ref.iter().map(|(ngram, count)| (*ngram, *count)),
