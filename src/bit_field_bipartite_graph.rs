...
impl WeightedBipartiteGraph for WeightedBitFieldBipartiteGraph {
    ...
    fn srcs_from_dst(&self, dst_id: usize) -> Self::Srcs<'_> {
        ...
        let keys: Vec<usize> = self.srcs_to_dsts.iter_from(start).take(end - start).collect();
        dbg!(keys.len(), start, end, self.srcs_to_dsts.iter_from(start).take(end - start).len());
